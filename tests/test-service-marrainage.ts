...
describe("Marrainage Service test", () => {
    ...
    beforeEach((done) => {
        ...
        differenceLodashSpy = sinon.spy(_, "differenceWith");
    ...
    describe("Test marrainage service with group", () => {
        ...
        it("should create marrainage with service group", async () => {
            ...
            let marrainageService = new MarrainageServiceWithGroup(
                testUsers.filter((u) => u.id === onboarder).map((u) => u.id),
                2
            );
            ...
            let marrainageGroup: MarrainageGroup = await knex(
                "marrainage_groups"
            )
                .where({
                    onboarder,
            ...
            marrainageGroup = await knex("marrainage_groups")
                .where({
                    onboarder,
                })
                .first();
            ...
            const marrainage_groups_members: MarrainageGroupMember = await knex(
            ...
            marrainageService = new MarrainageServiceWithGroup(
                testUsers
                    .filter((u) => u.id === "hela.ghariani")
                    .map((u) => u.id),
                2
            );
            ...
            chai.should().equal(
                marrainageGroup.status,
                MarrainageGroupStatus.PENDING
            );
        ...
        it("should set pending marrainage to status DOING if count > 1", async () => {
            ...
            let marrainageGroup = await knex("marrainage_groups")
                ...
                .returning("*")
                .then((res) => res[0]);
            await knex("marrainage_groups_members").insert({
                username: newcomer,
                marrainage_group_id: marrainageGroup.id,
            });
        ...
        it("should set pending marrainage to status DOING if created_date was 2 weeks ago", async () => {
            ...
            todayLessXdays.setDate(
                todayLessXdays.getDate() - (MARRAINAGE_GROUP_WEEK_LIMIT * 7 + 1)
            );
            let marrainageGroup = await knex("marrainage_groups")
                .insert({
                    onboarder,
