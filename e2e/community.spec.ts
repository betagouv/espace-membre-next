...
test("search valid community members", async ({ page }) => {
  ...
  const rows = await page.getByRole("table").locator("tbody").getByRole("row");

  await expect(await rows.count()).toEqual(3);

  await expect(await rows.nth(0).getByText("Another member")).toBeVisible();
  await expect(await rows.nth(2).getByText("Valid member")).toBeVisible();

  await page.getByText("Membres actifs uniquement").click();

  const rows2 = await page.getByRole("table").locator("tbody").getByRole("row");

  await expect(await rows2.count()).toEqual(6);
});
