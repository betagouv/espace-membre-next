...
export async function searchUsers(
  ...
  const query = db
    ...
    .select(({ eb, selectFrom }) => [
      ...
      sql<Array<string>>`array_agg(startups.name order by startups.name)`.as(
        "startups",
      ),
    ])
    .where(({ eb, ref }) => {
      const conditions: Expression<any>[] = [];
      const domaines = (filters.get("domaines")?.split(",").filter(Boolean) ||
        []) as UsersDomaineEnum[];
      const incubators =
      ...
      if (competences.length) {
        ...
        conditions.push(eb(ref("competences"), "?&", eb.val(competences)));
      }
