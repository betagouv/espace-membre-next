    ...
    try {
      ...
      if (axios.isAxiosError(e)) {
        ...
        if (e.response) {
          ...
          alert("Une erreur s'est produite, mais aucune réponse n'a été reçue");
        }
      } else {
    ...
    <>
      ...
      {calledOnce && !wasAlreadyConnected && (
        <div className="notification">
          <p>
            <b>
              Pour effectuer cette action il faut être connecter, nous allons
              t'envoyer un lien de connexion
            </b>
          </p>
          {!connected && !loginSent && (
            <form action="" method="POST" id="login_form" onSubmit={onSubmit}>
              <Input
          ...
          {loginSent && !connected && (
            <p>
              Lien de connexion envoyé ! Clique sur le lien de connexion que tu
              as reçu par email, puis sur "Me connecter" et reviens sur cette
              page.
              <br />
      ...
      {calledOnce && (
        <div style={!connected ? { opacity: 0.5, pointerEvents: "none" } : {}}>
          {props.children}
        </div>
      )}
    </>
...
const UserInfo = function (props: {
  ...
  return (
    <>
      ...
      <p>
        <span className="font-weight-bold">Rôle:</span> {props.userInfos.role}
      </p>
                ...
                <>
                  {mission.startups
                    ?.map((missionStartupUuid) =>
                      props.startups.find(
                        (startup) => startup.uuid === missionStartupUuid,
                      ),
                    )
...
const MemberComponent = function ({
  ...
  const showSteps =
    !!isExpired ||
    !hasEmailInfos ||
    userPublicInfos.primary_email_status === EmailStatusCode.EMAIL_SUSPENDED ||
    !!isEmailBlocked;
  if (!!isExpired) {
    ...
    <div>
      ...
      {showSteps && (
        <>
          ...
          <ul>
            {!!isExpired && (
              <li>
                Le contrat de {userPublicInfos.fullname} est arrivé à terme
            ...
            {userPublicInfos.primary_email_status ===
              ...
              <li>
                Son email @beta.gouv.fr est suspendu car sa date de fin a été
        ...
        <>
          <div className="notification">
            ...
            <ol>
              ...
              {userPublicInfos.primary_email_status ===
                EmailStatusCode.EMAIL_SUSPENDED && (
                <li>changer son mot de passe pour réactiver son email</li>
              )}
              {!!isEmailBlocked && (
                <li>
                  L'email est bloqué pour cause de spam, il faut le réactiver en
            ...
            {!hasEmailInfos && !!hasSecondaryEmail && (
              <p>
                Si tu es un collègue de {userPublicInfos.fullname} tu pourras
                recréer l'email pour lui/elle :).
              </p>
            )}
            {!hasEmailInfos && !!hasSecondaryEmail && (
              <p>
                Si tu es {userPublicInfos.fullname} tu pourras recréer l'email
                toi même une fois ta date de fin à jour.
              </p>
            )}
            {!hasEmailInfos && !hasSecondaryEmail && (
              <p>
                {userPublicInfos.fullname} n'a pas d'email secondaire, si tu es
                toi même {userPublicInfos.fullname} il va falloir qu'un collègue
                le fasse à ta place.
              </p>
            )}
          </div>
      ...
      {!showSteps && (
        <>
          ...
          <p>
            Cependant {userPublicInfos.fullname} rencontre peut être un des
            problèmes <span>suivants :</span>
          </p>
          <ul>
            <li>
              Je n'arrive pas à accéder a mon email @beta.gouv.fr/Mon mot de
        ...
        <>
          ...
          >
            <Button
              nativeButtonProps={{
                onClick: () => {
                  router.push(`/community/${userPublicInfos.username}`, {
                    scroll: false,
                  });
                },
              }}
...
export const UpdateEndDateScreen = function (props) {
  ...
  return (
    <ConnectedScreen title={title}>
      <h3>
        Mise à jour de la date de fin pour {props.user.userInfos.fullname}
      </h3>
      <div>
        {!!errorMessage && (
        ...
        <Input
          ...
          state={!!formErrors["nouvelle date de fin"] ? "error" : "default"}
        ...
        <div className="form__group">
          ...
          >
            <Button onClick={updateDate} disabled={isSaving} type="submit">
  ...
  return (
    <>
      {prStatus === "notMerged" && (
        <>
          <div>
            ...
            <a className="fr-link" href={pullRequestURL} target="_blank">
              {pullRequestURL}
            </a>
          </div>
          <p>
            Il faut la merger pour que le changement de date de fin soit prise
            en compte :
          </p>
      ...
      {prStatus === "merged" && (
        <>
          <p>
            La PR a probablement été mergée. Le changement sera pris en compte
            d'ici quelques minutes, il faut encore attendre :)
          </p>
          <a className="fr-link" href={pullRequestURL} target="_blank">
            {pullRequestURL}
          </a>
      ...
      {prStatus === "validated" && (
        <>
          <p>
            La date de fin est à jour c'est bon on peut passé à l'étape suivante
            :
          </p>
          <button className={"button"} onClick={() => next()}>
            Passer à l'étape suivante
...
export const WhatIsGoingOnWithMember = function (
  ...
  function next(steps?: STEP[], paramUser?: memberWrapperPublicInfoSchemaType) {
  ...
  if (step === STEP.whichMember) {
    ...
    stepView = (
      <div>
        <p>
          Il faut maintenant que {user.userPublicInfos.fullname} se connecte à{" "}
          <a className="fr-link" href="/account#password" target="_blank">
            l'espace-membre
          </a>{" "}
          avec son adresse secondaire.
        </p>
        <p>
          Un fois dans l'espace membre iel doit définir son mot de passe pour
          son adresse @beta.gouv.fr dans "changer mot de passe".
        </p>
        <button className="button" onClick={() => next()}>
          Passer à l'étape suivante
        </button>
    ...
    stepView = (
      <div>
        <p>
          La date de fin de mission de {user.userPublicInfos.fullname} a été
        ...
        <p>
          ...
          avec son adresse secondaire. Une fois dans l'espace membre iel doit
          définir son mot de passe pour son adresse @beta.gouv.fr dans "changer
          mot de passe".
        </p>
        <p>
          Iel aura alors de nouveau accès a son email en utilisant ce mdp dans
          son client email ou sur mail.ovh.net
        </p>
        <Button onClick={() => next()}>Passer à l'étape suivante</Button>
      </div>
    );
    ...
    stepView = (
      <div>
        <p>
          {user.userPublicInfos.fullname} a du faire un envoie massif d'email
          par gmail, ou depuis de nombreuses ips différentes. Son email a été
          bloqué par OVH.
        </p>
        <p>
          Pour le réactiver, iel doit se connecter a{" "}
    ...
    stepView = (
      <div>
        <p>
          Si {user.userPublicInfos.fullname} n'arrive plus accéder a son email
          @beta.gouv.fr, iel peut faire un changement de mot de passe.
        </p>
        <p>
      ...
      <div className="keskipasse-mattermost-container">
        ...
        <ol>
          ...
          <li>
            ...
            <ul>
              <li>
                Tente de te connecter avec ton mot de passe. <br />
                ⚠️ Le mot de passe de ton adresse @beta.gouv.fr et le mot de
                passe de mattermost sont deux mot de passe différents.
              </li>
              <li>
                Si tu as l'erreur suivante : "La connexion a échoué car le
                compte a été désactivé", il faut contacter les admins à
                l'adresse espace-membre@beta.gouv.fr, mettre en copie l'intra de
                startup et demander une réactivation du compte.
              </li>
              <li>
                Si tu as l'erreur suivante : "Spécifiez une adresse e­­-mail
                et/ou un mot de passe valide" ou "Trop grand nombre de tentative
                de connexion", c'est probablement que le mot de passe n'est pas
                bon, tu peux essayer de faire un renouvellement de mot de passe
                (voir point suivant)
              </li>
            </ul>
          </li>
          <li>
            Si tu n'as pas de mot de passe ou que tu n'arrives toujours pas a te
            connecter :
            <ul>
              <li>
              ...
              <li>
                Tu devrais recevoir un email pour redéfinir ton mot de passe.
                <br />
                ⚠️ L'email est peut-être dans les spams.
                <br />
                ⚠️ Si tu utilises gmail, les emails peuvent arriver avec un
                délai. Pour les récupérer instantanément aller dans Paramètres
                ⚙️ → comptes et importation → Consulter d'autres comptes de
                messagerie → Consulter votre messagerie maintenant.
              </li>
            </ul>
          </li>
          <li>
            Si le problème n'est toujours pas réglé envoie un email à
            espace-membre@beta.gouv.fr avec en copie l'intra ou un membre de
            l'équipe d'animation de beta que tu connais
          </li>
        </ol>
      </div>
    );
    ...
    stepView = (
      <div>
        ...
        <p>
          ...
          <br />
          2. ⚠️ Si {user.userPublicInfos.fullname} utilises gmail, les emails
          peuvent arriver avec un délai. Pour les récupérer instantanément aller
          dans Paramètres ⚙️ → comptes et importation → Consulter d'autres
          comptes de messagerie → Consulter votre messagerie maintenant.
          <br />
          3. Il peut y avoir un délai de latence entre l'envoie et la réception
          de l'email, si il s'est passé moins de 20 minutes depuis votre
          tentative de connexion, attendre 20 minutes et revérifier l'étape 1 et
          2.
        </p>
        <p>
          Si toujours pas d'email, l'email de {user.userPublicInfos.fullname}{" "}
          est probablement bloqué par brevo notre service d'email. Merci de nous
          faire part du soucis dans le chat :
          <br />
          <br />
          <Button
