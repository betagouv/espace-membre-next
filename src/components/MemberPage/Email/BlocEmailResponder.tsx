...
export default function BlocEmailResponder({
  ...
  return (
    <Accordion label="Configurer une réponse automatique">
      ...
      <p>
        Informez vos correspondants de votre absence. Cette réponse automatique
        sera envoyée à tous les messages que vous recevez. (la mise en place
        effective de la réponse automatique peut prendre quelques minutes)
      </p>

      <form className="fr-mb-6v" onSubmit={handleSubmit(onSubmit)}>
        {responder && <input type="hidden" name="method" value="update" />}
        <Input
        ...
        <Button
          ...
          children={isSaving ? `Sauvegarde en cours...` : `Sauvegarder`}
        />
      </form>
    </Accordion>
  );
