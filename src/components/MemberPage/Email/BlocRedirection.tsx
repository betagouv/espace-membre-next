...
export default function BlocRedirection({
  ...
  return (
    <Accordion
      label={
        <span id="redirection">Rediriger vers une autre adresse mail</span>
      }
    >
      <p>
        🚨 Il n'est pas recommandé d'utiliser les redirections :{" "}
        <b>certains de tes messages seront perdus</b>. Mais une redirection peut
        être utile en complément d'une récupération <i>POP</i> ou d'une
        application type Frontapp.
      </p>
          ...
          <div className="redirection-item" key={i}>
            ...
            {canCreateRedirection && (
              <form
                onSubmit={async (e) => {
                  e.preventDefault();
                  const resp = await safeDeleteRedirectionForUser({
      ...
      {canCreateRedirection && (
        ...
        >
          ...
          <Button
            ...
            children={isSaving ? `Ajout en cours...` : `Ajouter la redirection`}
          />
        </form>
      )}
      {canCreateRedirection && (
        <>
          ...
          {!isExpired && (
            <div className="notification warning">
              Seul {userInfos.fullname} peut créer ou modifier les redirections.
            </div>
          )}
        </>
      )}
