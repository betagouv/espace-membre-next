...
export default ({ redirections, canCreateRedirection, userInfos, isExpired, domain }) => {
    const [toEmail, setToEmail] = React.useState<string>('')
    const [keepCopy, setKeepCopy] = React.useState<boolean>('')
    console.log(redirections)
    return <Accordion label="Rediriger vers une autre adresse mail">
        ...
        { (redirections || []).map((redirection) => {
            return <div className="redirection-item">
                {redirection.to}
                {canCreateRedirection &&
                    <form
                        ...
                        onSubmit={(e) => {
                            e.preventDefault()
                            axios.delete(routes.USER_DELETE_REDIRECTION
        ...
        {canCreateRedirection &&
            <form
                onSubmit={
                    (e) => {
                        e.preventDefault()
                        axios.post(routes.USER_CREATE_REDIRECTION.replace(':username', userInfos.id), {
                        toEmail,
                        keepCopy,
                    })
                    }
                ...
                <Button 
                    ...
                    nativeButtonProps={{
                        type: 'submit',
                    }}>Ajouter la redirection</Button>
            </form>
        }
        {canCreateRedirection && <>
            { isExpired && <div className="notification error">
