...
export default function BlocConfigurerCommunicationEmail({
    ...
    const [ value, setValue ] = React.useState<"primary" | "secondary" | undefined>(communication_email);

    return <Accordion label="Quel email utiliser pour les communications @beta.gouv.fr ?">
        ...
        {(!!primaryEmail && !!secondaryEmail) && <form
        method="POST" onSubmit={
            () => {
                axios.put(routes.USER_UPDATE_SECONDARY_EMAIL, {
                    communication_email
                })
            }
        }>
