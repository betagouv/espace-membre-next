...
export default function BlocChangerMotDePasse({ canChangePassword, emailSuspended, userInfos }) {
    const [password, setPassword] = React.useState<string>('')
    return <Accordion label="Changer mon mot de passe">
        {(canChangePassword) && <>
            <form 
                onSubmit={(e) => {
                    e.preventDefault()
                    axios.post(routes.USER_UPDATE_PASSWORD.replace(':username', userInfos.id), {
