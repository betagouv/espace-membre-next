    ...
    <div className={fr.cx("fr-mb-3w")}>
      ...
      <Table
        ...
        data={phasesFields.map((phase, index) => {
          const startDateString = getValues(`startupPhases.${index}.start`)
          ...
          return [
            <Select
              key={phase.id + "-name"}
              label={undefined}
              nativeSelectProps={register(`startupPhases.${index}.name`)}
            >
              <option value="" disabled hidden>
                Selectionnez une phase
              </option>
              {Object.entries(PHASES_ORDERED_LIST).map(([id, label], index) => (
