    ...
    <>
      {match(account)
        .with(
          ...
          () => (
            <>
              <>
                <b>Statut</b> :{" "}
                {account.status === ACCOUNT_SERVICE_STATUS.ACCOUNT_FOUND && (
              ...
              {data && !!data.length && <Table data={data} headers={headers} />}
              {(!data || !data.length) && !nbEvents && (
                <p className="fr-mt-4w">
                  Ton compte existe mais tu n'as encore accès à aucun site ou
                  équipe, tu peux en faire la demande ci-dessous
                </p>
              )}
            </>
          ),
        ...
        .with(
          ...
          () => (
            <Alert
              description={"Une invitation t'a été envoyée par email."}
              small={true}
              closable={false}
              severity="info"
            />
