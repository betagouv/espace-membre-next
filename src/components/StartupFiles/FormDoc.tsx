  ...
  return (
    ...
    >
      ...
      {isComiteSchema(schema) && (
        <>
          {schema.shape.data.shape.date_comite && (
            <Input
              label={schema.shape.data.shape.date_comite.description}
              nativeInputProps={{
          ...
          {schema.shape.data && typeof schema.shape.data === "object" && (
            <Checkbox
              small
              legend={schema.shape.data.shape.contenu.description}
              state={
                // @ts-ignore todo
              ...
              options={contenusTypes.map((m, i) => ({
                ...
                nativeInputProps: {
                  value: m.label,
                  checked: selectedContenu.includes(m.label),
      ...
      <Button
        ...
        iconId={isSubmitting ? "fr-icon-timer-fill" : "fr-icon-add-circle-fill"}
        children={isSubmitting ? `Enregistrement en cours...` : `Enregistrer`}
        nativeButtonProps={{
          type: "submit",
