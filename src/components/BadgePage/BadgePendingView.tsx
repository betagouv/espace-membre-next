...
export const BadgePendingView = ({
  ...
  function getStatus(dossier: badgeDossierSchemaType) {
    ...
    const status: string = statusObject ? statusObject.stringValue : "ERROR";
    return (
      <>
        <p>
          <b>Statut du badge :</b>
  ...
  return (
    <>
      {dsToken && !dossier && (
        <>
          <p>
            Votre démarche a été préremplie, vous pouvez maintenant cliquer sur
            le bouton ci-dessous pour la terminer.
          </p>
          <p>⚠️ Veillez à respecter les règles suivantes :</p>
          <ul>
            <li>
              Le compte à utiliser pour remplir la demande doit être lié à votre
              adresse : {primaryEmail}
            </li>
            <li>
