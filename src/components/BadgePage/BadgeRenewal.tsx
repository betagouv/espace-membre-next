...
export default function BadgeRenewal({
  ...
  function askForBadge() {
    ...
    setIsSaving(true);
    axios
      .post(computeRoute(routes.API_POST_BADGE_RENEWAL_REQUEST), undefined, {
  ...
  return (
    <>
      ...
      {!!dossier && (
        <>
          ...
          {!forceRequest && (
            <>
              <p>
                Si tu penses qu'il y a une erreur, tu peux refaire une demande.
              </p>
              <Button
                priority="secondary"
                nativeButtonProps={{
      ...
      {(!dossier || forceRequest) && (
        <>
          <p>
            Tu t'apprêtes à faire une demande de renouvellement de badge.
            <br />
            C'est donc que tu as actuellement un badge mais qu'il ne fonctionne
            plus. Pour prolonger ton badge, il faut que tu viennes au minimum
            deux fois par semaine.
          </p>
          <p>
            Sinon tu devras demander un badge invité à chacune des tes venues.
            Ce sont les régles du batiment Ségur nous n'y pouvont rien.
          </p>
