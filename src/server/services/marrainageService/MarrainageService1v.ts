...
export class MarrainageService1v implements MarrainageService {
    private _sendOnboarderRequestEmail: (
        newcomerId: string,
        onboarderId: string
    ) => Promise<null>;
    senderEmail: string;
    ...
    async selectRandomOnboarder(newcomerId, domaine) {
        ...
        const existingCandidates: string[] = await db("marrainage")
            ...
            .distinct()
            .then((marrainages: Marrainage[]) =>
                marrainages.map((x) => x.last_onboarder)
            );
        ...
        const onboardersFromDomaine = onboarders.filter(
            (onboarder) => onboarder.domaine === domaine
        );
        const onboarderPool =
            domaine === Domaine.AUTRE || !onboardersFromDomaine.length
                ? onboarders
                : onboardersFromDomaine;
        return onboarderPool[Math.floor(Math.random() * onboarderPool.length)];
    }

    async createMarrainage(
        newcomerId: string,
        domaine: string
    ): Promise<Member> {
    ...
    async updateMarrainage(newcomer: Member): Promise<Member> {
        ...
        console.log(
            `Select ${onboarder.id} (${onboarder.domaine} for ${newcomer.id} (${newcomer.domaine}))`
        );
    ...
    async getUsersWithoutMarrainage(): Promise<DBUser[]> {
        ...
        const marrainages = await knex("marrainage").whereIn(
            "username",
