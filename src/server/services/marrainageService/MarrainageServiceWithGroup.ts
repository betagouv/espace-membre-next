...
import {
    MarrainageGroupStatus,
    MarrainageGroup,
    MARRAINAGE_EVENT,
} from "@/models/marrainage";
...
export class MarrainageServiceWithGroup implements MarrainageService {
    ...
    async selectRandomOnboarder(): Promise<Member> {
        let pendingMarrainageGroup: MarrainageGroup = await knex(
            "marrainage_groups"
        )
            .where({
        ...
        if (pendingMarrainageGroup) {
            onboarder = await betagouv.userInfosById(
                pendingMarrainageGroup.onboarder
            );
        } else {
            const marrainageGroups: MarrainageGroup[] = await knex(
                "marrainage_groups"
            );
            const onboarders: string[] = marrainageGroups.map(
                (marrainageGroup) => marrainageGroup.onboarder
            );
            // we take the onboarder with less marrainages
            const sortedOnboarder = countNumberOfMarrainage([
                ...onboarders,
                ...this.users,
            ]).sort(sortAscending);
            onboarder = await betagouv.userInfosById(
                sortedOnboarder[0].onboarder
            );
        }
        return onboarder;
    }
    ...
    async createMarrainage(newcomerId): Promise<Member> {
        ...
        await knex.transaction(async (trx) => {
            ...
            await trx("marrainage_groups_members").insert({
                username: newcomerId,
    ...
    async _setMarrainageToDoing(marrainage_group_id) {
        ...
        console.info(
            `MarrainageGroup ${marrainage_group_id} status set to DOING`
        );
        EventBus.produce(MARRAINAGE_EVENT.MARRAINAGE_IS_DOING_EVENT, {
    ...
    async checkAndUpdateMarrainagesStatus() {
        ...
        todayLessXdays.setDate(
            todayLessXdays.getDate() - this.MARRAINAGE_GROUP_WEEK_LIMIT * 7
        );
        const marrainage_groups: MarrainageGroup[] = await knex(
            "marrainage_groups"
        )
            .where({
                status: MarrainageGroupStatus.PENDING,
    ...
    async getUsersWithoutMarrainage(): Promise<DBUser[]> {
        ...
        const marrainages = await knex("marrainage_groups_members").whereIn(
            "username",
            users.map((user) => user.username)
        );
        const oldMarrainages = await knex("marrainage").whereIn(
            "username",
            users.map((user) => user.username)
        );
        ...
        return _.differenceWith(
            users,
            [...marrainages, ...oldMarrainages],
            (user, marrainage) => user.username === marrainage.username
        );
    }
}
