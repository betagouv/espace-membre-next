...
export function TeamCompositionEmail(props: EmailTeamComposition["variables"]) {
  ...
  return (
    <StandardLayout title={title}>
          ...
          <tbody>
            {props.activeMembers.map((activeMember, index) => (
              <tr key={index}>
                ...
                <td style={{ width: "35%" }}>
                  Début :{" "}
                  {format(activeMember.activeMission.start, "dd/MM/yyyy", {
                    locale: fr,
                  })}{" "}
                  <br />
                  Fin:{" "}
                  {!!activeMember.activeMission.end
                    ? format(activeMember.activeMission.end, "dd/MM/yyyy", {
                        locale: fr,
                      })
                    : "Pas de date de fin"}
                </td>
      ...
      <MjmlText>
        <p>
          Si tes informations ne sont pas à jour, tu peux les modifier en
