...
export async function processNotification(req, res) {
    ...
    const emailData = details.reduce(
        (result, item) => {
            const dates: any = extractEndDates(item);
            if (dates.before < dates.after) {
                const formatDate = (d) => d.toISOString().slice(0, 10);
                const msg = `Le badge de ${
                    item.after.attributes.fullname
                } est aujourd'hui valable jusqu'au ${formatDate(
                    dates.before
                )}. Il doit être prolongé jusqu'au ${formatDate(dates.after)}.`;
    ...
    if (emailData.messages.length) {
      ...
      <p>Merci.</p>`;
            ...
            await sendMail(
                emailData.to,
                "Demande de prolongation d’un badge",
                emailContent
            );
        } else {
            const emailContent = `
      ...
      <p>Merci.</p>`;
            ...
            await sendMail(
                emailData.to,
                `Demande de prolongation de ${emailData.messages.length} badges`,
                emailContent
            );
        }
    }
