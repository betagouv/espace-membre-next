...
import {
    MarrainageService1v,
    MarrainageServiceWithGroup,
} from "@services/marrainageService";
import { sendEmail } from "@config/email.config";
import { EMAIL_TYPES } from "@modules/email";
import { sendOnboarderRequestEmail } from "@modules/marrainage/eventHandlers";

const useNewMarrainage =
    config.FEATURE_USE_NEW_MARRAINAGE && config.MARRAINAGE_ONBOARDER_LIST;
const MarrainageService = useNewMarrainage
    ? new MarrainageServiceWithGroup(
          config.MARRAINAGE_ONBOARDER_LIST,
          config.MARRAINAGE_GROUP_LIMIT
      )
    : new MarrainageService1v(config.senderEmail, sendOnboarderRequestEmail);
...
export async function reloadMarrainage(newcomerId) {
    ...
    const onboarder: Member = await (
        MarrainageService as MarrainageService1v
    ).updateMarrainage(newcomer);
    console.log(
...
export async function createRequestForUser(userId) {
    ...
    const onboarder = await MarrainageService.createMarrainage(
        newcomer.id,
        newcomer.domaine
    );
    return {
...
export async function acceptRequest(req, res) {
    try {
        ...
        const dbUsers: DBUser[] = await knex("users").whereIn("username", [
            onboarder.id,
            newcomer.id,
        ]);
        const dbOnboarder: DBUser = dbUsers.find(
            (user) => user.username === onboarder.id
        );
        const dbNewcomer: DBUser = dbUsers.find(
            (user) => user.username === newcomer.id
        );
        try {
            const emailOnboarder =
                dbOnboarder.communication_email ===
                    CommunicationEmailCode.SECONDARY &&
                dbOnboarder.secondary_email
                    ? dbOnboarder.secondary_email
                    : dbOnboarder.primary_email;
            const emailNewcomer =
                dbNewcomer.communication_email ===
                    CommunicationEmailCode.SECONDARY &&
                dbNewcomer.secondary_email
                    ? dbNewcomer.secondary_email
...
export async function reloadRequest(req, res) {
    try {
        const { newcomer, onboarder } = await reloadMarrainage(
            req.body.newcomerId
        );
