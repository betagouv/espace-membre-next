...
import {
    EventEmailMarrainageNewcomer,
    MARRAINAGE_EVENT,
} from "@models/marrainage/marrainageEvent";
...
describe("Test marrainage send newcomer email", () => {
    it("should send email to newcomer email", async () => {
        ...
        const [marrainage_group] = await db("marrainage_groups")
            .insert({
        ...
        const email: EmailProps = {
            ...
            toEmail: [
                dbUser.communication_email ===
                    CommunicationEmailCode.SECONDARY && dbUser.secondary_email
