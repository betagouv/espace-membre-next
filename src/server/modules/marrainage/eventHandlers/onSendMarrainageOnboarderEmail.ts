...
export const sendOnboarderRequestEmail = async function (
    newcomerId: string,
    onboarderId: string
) {
    ...
    const startup: string | null =
        newcomer.startups && newcomer.startups.length > 0
            ? newcomer.startups[0]
            : null;

    const dbOnboarder: DBUser = await knex("users")
        .where({
    ...
    try {
        const email =
            dbOnboarder.communication_email ===
                CommunicationEmailCode.SECONDARY && dbOnboarder.secondary_email
