...
export const onMarrainageSendOnboarderEmail: (
    evenement: EventEmailMarrainageOnboarder
) => Promise<void> = async (evt) => {
    ...
    if (!onboarder) {
        console.error(
            `Error : user ${evt.user} does not exist in db cannot send email`
        );
        return;
    }
    ...
    const newcomers: (DBUser & MarrainageGroupMember)[] = await db(
    ...
    const email: EmailProps = {
        ...
        toEmail: [
            onboarder.communication_email ===
                CommunicationEmailCode.SECONDARY && onboarder.secondary_email
                ? onboarder.secondary_email
        ...
        variables: {
            ...
            newcomers: newcomers.map((newcomer) => ({
                ...
                email: newcomer.communication_email
                    ? newcomer.primary_email
                    : newcomer.secondary_email,
