...
const sendEmailToAuthorsIfExists = async (author, pullRequestNumber) => {
    ...
    if (!user) {
        ...
        console.log(
            `Message de rappel de pr envoyé par email à ${user.username}`
        );
    }
};
    ...
    for (const author of authors) {
        ...
        if (startup && startup.mailing_list) {
            ...
            try {
                ...
                console.error(
                    `Erreur lors de l'envoie d'un email à ${author}`,
                    e
                );
            }
        }
...
const sendMessageToAuthorsIfAuthorFilesInPullRequest = async (
    ...
    for (const author of authors) {
        ...
        try {
            await sendMattermostMessageToAuthorsIfExists(
                author,
                pullRequestNumber
            );
        } catch (e) {
...
const filterUpdateDateXdaysAgo = (createdDate, nbOfDays) => {
    ...
    return (
        createdDate < thresholdDate && createdDate > thresholdDateLessOneHour
    );
};
