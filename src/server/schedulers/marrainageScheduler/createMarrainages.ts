...
export async function createMarrainages() {
    ...
    const userWithoutMarrainage: DBUser[] =
        await MarrainageService.getUsersWithoutMarrainage();
    for (const user of userWithoutMarrainage) {
        try {
            ...
            console.error(
                `Impossible de créer un marrainage pour ${user.username}`
            );
