...
export async function sendMessageToTeamForJobOpenedForALongTime(
    jobs = undefined
) {
    console.log(
        `Lancement du cron pour les messages de rappel de fermeture d'offre`
    );
    if (!jobs) {
    ...
    for (const job of jobs) {
        const startup = startups_details.find(
            (startup) => startup.id === job.startup
        );
        if (!startup) {
            continue;
        }
        ...
        if (!contact) {
            contact = startup.active_members
                .map((active_member) => {
        ...
        if (contact) {
            const mattermostUser: MattermostUser = await getUserByEmail(
                buildBetaEmail(contact.id)
            );
            if (mattermostUser) {
                ...
                console.log(
                    `Message de rappel pour fermer l'offre envoyer à ${mattermostUser.username}`
                );
            }
        }
    }
