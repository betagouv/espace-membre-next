...
describe("Marrainage", () => {
            ...
            knex("marrainage")
                ...
                .then(() => {
                    const token = jwt.sign(
                        { newcomerId, onboarderId },
                        config.secret
                    );

                    chai.request(app)
                        .get(
                            `/marrainage/accept?details=${encodeURIComponent(
                                token
                            )}`
                        )
                        .redirects(0)
                        .end((err, res) => {
                            ...
                            const email1: SendEmailProps =
                                sendEmailStub.args[0][0];
                            const email2: SendEmailProps =
                                sendEmailStub.args[1][0];
                            email1.type.should.equal(
                                EMAIL_TYPES.MARRAINAGE_ACCEPT_ONBOARDER_EMAIL
                            );
                            email2.type.should.equal(
        ...
        it("should select and email a new onboarder if declined", (done) => {
            ...
            knex("marrainage")
                ...
                .then(() => {
                    ...
                    chai.request(app)
                        .get(
                            `/marrainage/decline?details=${encodeURIComponent(
                                token
                            )}`
                        )
                        .redirects(0)
                        .end((err, res) => {
                            res.should.have.status(200);
                            res.text.should.include(
                                "Votre décision a été prise en compte"
                            );
                            sendEmailStub.calledOnce.should.be.true;

                            const email: SendEmailProps =
                                sendEmailStub.args[0][0];
                            email.type.should.equal(
    ...
    describe("authenticated", () => {
        ...
        it("email should include role only when startup not available", async () => {
            ...
            await chai.request(app).post("/marrainage").type("form").send({
            ...
            email.variables.newcomer.role.should.equal(
                testUsers.find(
