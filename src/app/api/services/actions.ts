...
const createOrUpdateSentryAccount = async (
  ...
  const allRequestedStartupAreAuthorized = _.every(startupsByTeamName, (item) =>
    _.includes(userStartups, item),
  );
  ...
  const canRequestAccessToStartup: boolean =
    isAdmin || (allRequestedStartupAreAuthorized && newTeamStartupIsAuthorized);
