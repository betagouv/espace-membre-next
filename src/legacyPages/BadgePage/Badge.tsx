...
export const WelcomeScreen = function (props) {
    ...
    function askForBadge() {
        if (props.dossier || props.badgeRequest) {
            ...
            setIsSaving(true);
            axios
                .post(computeRoute(routes.API_POST_BADGE_REQUEST))
                .then((resp) => {
                    setIsSaving(false);
        ...
        stepView = (
            <>
                ...
                {dsToken && !props.dossier && (
                    <>
                        <p>
                            Votre démarche a été préremplie, vous pouvez
                            maintenant cliquer sur le bouton ci-dessous pour la
                            terminer.
                        </p>
                        <p>⚠️ Veillez à respecter les règles suivantes :</p>
                        <ul>
                            <li>
                                Le compte à utiliser pour remplir la demande
                                doit être lié à votre adresse :{" "}
                                {props.primaryEmail}
                            </li>
                            <li>
                                La date de fin de votre badge a été préremplie,
                                si vous la changez, mettez maximum 1 an{" "}
                            </li>
                        </ul>
                        <a
                            className="button"
                        ...
                        >
                            J'ai compris. Poursuivre la démarche sur
