...
export async function postLoginApi(req, res) {
    ...
    if (emailSplit[1] === config.domain) {
        ...
        if (
            ...
            return res.json(
                {
                    message: `Le nom de l'adresse email renseigné n'a pas le bon format. Il doit contenir des caractères alphanumériques en minuscule et un '.' Exemple : charlotte.duret@${config.domain}`,
                },
                { status: 500 }
            );
        }
    ...
    if (!dbResponse) {
        return res.json(
            {
                message: `L'adresse email ${emailInput} n'est pas connue.`,
            },
            {
                status: 404,
            }
    ...
    if (
        ...
        return res.json(
            {
                message: `La personne liée à l'adresse ${emailInput} n'a pas un compte actif. Réglez le problème en utilisant l'interface de diagnostic https://espace-membre.incubateur.net/keskispasse`,
            },
            { status: 403 }
        );
    }
