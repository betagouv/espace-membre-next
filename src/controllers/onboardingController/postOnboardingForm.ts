...
export async function postForm(req, res) {
    ...
    try {
        ...
        const start =
            req.body.start ||
            requiredError("début de la mission", errorHandler);
        const end =
        ...
        const should_create_marrainage =
            req.body.should_create_marrainage || false;
        const hasPublicServiceEmail = await utils.isPublicServiceEmail(
            inputEmail
        );
