...
export type ComponentProps<F extends (props: any) => JSX.Element> = F extends (
    ...args: infer A
) => any
    ? Partial<A[0]>
    : never;
...
export function disableGlobalDsfrStyle(value: boolean): void {
    ...
    const styleElements = document.querySelectorAll(
        'head > meta[name="next-head-count"] ~ style:not([data-s])'
    );
        ...
        styleElements.forEach((styleElement) => {
            if (
                !styleElement.innerHTML.includes(
                    "protectedFromStyleSwitchWorkaround"
                )
            ) {
...
export interface StoryHelpers<ComponentType> {
    generateMetaDefault: (
        initialMeta: Meta<ComponentType>
    ) => Meta<ComponentType>;
    prepareStory: <
        ComponentType,
        ContextValueType extends MatchingContextValue<PossibleContextValueType>,
        PossibleContextValueType
    >(
...
export function StoryHelperFactory<
    ComponentType
>(): StoryHelpers<ComponentType> {
    return {
        generateMetaDefault,
        prepareStory,
    };
...
export function generateMetaDefault<ComponentType>(
    initialMeta: Meta<ComponentType>
): Meta<ComponentType> {
...
export type MatchingContextValue<PossibleContextValueType> = {
    [key in keyof PossibleContextValueType]:
        | PossibleContextValueType[key]
        | StoryFn<PossibleContextValueType[key]>;
};
...
>(
    ...
    if (options?.layoutStory) {
        ...
        if (options.layoutStory.parameters?.nextAuthMock) {
            ...
            if (
                options.layoutStory.parameters?.nextjs?.navigation &&
                !story.parameters?.nextjs?.navigation
            ) {
            ...
            story.parameters.nextAuthMock = {
                ...options.layoutStory.parameters.nextAuthMock,
                ...story.parameters.nextAuthMock,
            };
        }
        ...
        if (LayoutStory.parameters?.msw) {
            ...
            story.parameters.msw.handlers = [
                ...LayoutStory.parameters.msw.handlers,
                ...story.parameters.msw.handlers,
            ];
        }
    }
    ...
    if (options?.childrenContext) {
            ...
            for (const key in options.childrenContext.value) {
                ...
                if (
                    ...
                    if (childStory.parameters?.msw) {
                        ...
                        story.parameters.msw.handlers = [
                            ...childStory.parameters.msw.handlers,
                            ...story.parameters.msw.handlers,
                        ];
                    }
        ...
        const safeValues = options.childrenContext
            .value as unknown as PossibleContextValueType & object;
