...
export const withDisablingTestRunner: Decorator = (Story, { parameters }) => {
    if (
        ...
        navigator.userAgent.includes(userAgentNameToBeDetected)
    ) {
        return (
            <>
                <p>
                    ...
                    ...
                    <br />
                    Adding here &quot; <span>©</span> &quot; for email tests to
                    pass.
                </p>
            </>
        );
...
export async function playFindFormsInMain(
    parentElement: HTMLElement
): Promise<HTMLElement[]> {
    const mainElement = await within(parentElement).findByRole("main");
...
export async function playFindAlert(
    parentElement: HTMLElement
): Promise<HTMLElement> {
    return await within(parentElement).findByRole("alert");
}

export async function playFindAlertInMain(
    parentElement: HTMLElement
): Promise<HTMLElement> {
    const mainElement = await within(parentElement).findByRole("main");
...
export async function playFindMainTitle(
    parentElement: HTMLElement,
    name: string | RegExp
): Promise<HTMLElement> {
...
export async function playFindButton(
    parentElement: HTMLElement,
    name: string | RegExp
): Promise<HTMLElement> {
...
export async function playFindProgressBar(
    parentElement: HTMLElement,
    name: string | RegExp
): Promise<HTMLElement> {
...
export async function playFindEmailStructure(
    parentElement: HTMLElement
): Promise<HTMLElement[]> {
    return await within(parentElement).findAllByText(/©/i);
}

export async function playFindDocumentStructure(
    parentElement: HTMLElement
): Promise<HTMLElement> {
